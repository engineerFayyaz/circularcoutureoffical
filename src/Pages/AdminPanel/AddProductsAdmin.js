...
const [selectedEditName, setSelectedEditName] = useState("");
  ...
  const handleInputChange = (e) => {
    ...
    if (name === "rrp") {
      ...
      // Calculate 8 day 30 % Discount
      const discount8Days = Math.round(perDayRate4Days * 0.3);
      const perDayRate8Days = perDayRate4Days - discount8Days;
  
      // Calculate Rent Price (8 days) 30% discount on day rate
      const rentPrice8Days = Math.round(perDayRate8Days * 8);
  
      // Calculate 16 day 50 % Discount
      const discount16Days = Math.round(perDayRate4Days * 0.5);
      const perDayRate16Days = perDayRate4Days - discount16Days;
  
      // Calculate Rent Price (16 days) 50% discount on day rate
      const rentPrice16Days = Math.round(perDayRate16Days * 16);
  
      // Calculate 30 day 70 % Discount
      const discount30Days = Math.round(perDayRate4Days * 0.7);
      const perDayRate30Days = perDayRate4Days - discount30Days;
  
      // Calculate Rent Price (30 days) 70% discount on day rate
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const requestData = {
        ...
        editId: selectedEditId, // Add editId to requestData
            ...
            <Col lg={5} className="mt-4">
              <Row className="order-page-admin">
                <Form onSubmit={handleSubmit}>
                  ...
                  <Form.Group className="mb-3 mt-3 Category-admin">
                    <Form.Label><b>Search Designer*</b></Form.Label>
                    <Form.Control
                      as="select"
                  ...
                  <Form.Group className="mb-3 mt-3 Category-admin">
                    <Form.Label><b>Selected Designer</b></Form.Label>
                    <Form.Control
                      type="text"
                      ...
                      style={{ display: "none" }}
                    />
                    <input type="hidden" name="designerId" value={selectedDesignerId} />
                    <p>{selectedDesignerName}</p>
                  </Form.Group>

                  <Form.Group className="mb-3 mt-3 Category-admin">
                    <Form.Label><b>Select Edit*</b></Form.Label>
                    <Form.Select
                      name="editId"
                      ...
                      onChange={(e) => {
                        ...
                        const selectedEdit = edits.find((edit) => edit.id === parseInt(selectedId));
                        if (selectedEdit) {
                  ...
                  <Form.Group className="mb-3 mt-3 Category-admin">
                    <Form.Label><b>Selected Edit</b></Form.Label>
                    <Form.Control
                      type="text"
                      ...
                      style={{ display: "none" }}
                    />
                    <input type="hidden" name="editId" value={selectedEditId} />
                    <p>{selectedEditName}</p>
                  </Form.Group>
                  ...
                  <Form.Group className="mb-3 mt-3 Category-admin">
                    <Form.Label><b>Selected Images</b></Form.Label>
                    <div>
                      {selectedImageURLs.map((url, index) => (
                        ...
                        >
                          ...
                          <span style={{ marginLeft: "10px" }}>✔️</span> {/* Tick symbol */}
                        </div>
                      ))}
                    </div>
                  </Form.Group>
